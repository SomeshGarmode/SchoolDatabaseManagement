package com.school;

import java.io.IOException;
import java.io.PrintWriter;

import javax.persistence.EntityManager;
import javax.persistence.EntityManagerFactory;
import javax.persistence.EntityTransaction;
import javax.persistence.Persistence;
import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
@WebServlet("/tfetch")
public class Teacher_Fetch extends HttpServlet{

	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		String tid = req.getParameter("tid");	
		String tname = req.getParameter("tname");
		int tid1 = Integer.parseInt(tid);
		
	  		
		EntityManagerFactory emf = Persistence.createEntityManagerFactory("cshool");
		EntityManager em = emf.createEntityManager();
		
		
		Teacher t=em.find(Teacher.class,tid1);
                                    //		 t.setTid(tid1);t.setTname(tname);t.setSubject(subject);t.setSalary(salary1);
		System.out.println(t.getTid());
		System.out.println(t.getTname());
		System.out.println(t.getSalary());
		
		System.out.println(t.getSubjects());
		
		PrintWriter pw = resp.getWriter();
		pw.write("fetched teacher  Data");
		pw.write(t.getTid());
		pw.write(t.getTname());
		pw.write(t.getSubjects());
		                          //pw.write(t.getSalary());
		RequestDispatcher rd = req.getRequestDispatcher("Teacher.html");
		rd.forward(req, resp);
		
	}
}
